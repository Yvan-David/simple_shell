...
char **_which(char *foundpath)
{
	...
	copy_path = _strdup(foundpath); /*copies the string*/
	tokens = strtok(copy_path, sparse); /*separates the string into tokens*/
	while (tokens != NULL)
	{
		dir[i] = tokens;
		i++;
 ...
 * Return: 1
 */
int child_process(char **av, char **args, char **env, int status_main, int ct)
{
	pid_t pid;
	int status;
